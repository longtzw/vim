...
查找命令'/'抵达文档结尾处时会回绕至文档开头继续查找，如果不想回绕，可以关闭'wrapscan'选项。

**回溯之前的查找**

我们可以通过<UP>键，滚动浏览之前的查找记录

### 5.2.2 在执行查找前预览第一处匹配

缺省情况下, 我们在输入查找模式时, Vim不会进行查找, 只有当我们按下<CR>后, 它才会立即展开行动, 'incsearch'会让Vim根据已在
查找域中输入的文本, 预览第一处匹配。

**根据预览结果对查找域自动补全**

<C-r><C-w>会用当前预览的匹配结果对查找域进行自动补全。
...
标志位n会抑制正常的替换动作, 而是简单地统计匹配次数。

### 5.2.4 利用查找历史, 迭代完成复杂模式

'q/'可以调出查找历史窗口, 在此窗口下可以使用Vim强大的区分模式的编辑能力。
...
输入<C-r>/ 即可把上次的查找内容粘贴进来。

### 5.2.7 用寄存器的值替换

**引用**

假设我们已经复制了多行文本, 并存放于寄存器0中。运
行下面的命令，我们可以在substitute命令的替换域中使用这段文本

```
:%s//\=@0/g
```

\= 将指示Vim执行一段表达式脚本。在Vim脚本中，我们可以用@{register}来引用这个寄存器的内容


### 5.2.8 重复上一次substitute命令

:&& 命令中前一个&作为Ex命令 :& 的组成部分，用作重复上一次的 :substitute 命令，而第二个&则会重用上一次 :s 命令的标志位。
